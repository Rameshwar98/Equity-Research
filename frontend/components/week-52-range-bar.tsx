...
export function Week52RangeBar({
  ...
  return (
    <div
      className="mx-auto w-[min(100%,8.75rem)] min-w-[6.75rem] px-0.5 py-0.5"
      title={`Last ${fmt(last)} · 52W ${fmt(low)} – ${fmt(high)}`}
    >
      <div className="flex justify-between gap-0.5 text-[8px] tabular-nums leading-none mb-0.5">
      ...
      <div className="relative h-3.5 flex items-center">
        ...
        <div
          className="absolute top-1/2 z-10 size-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground shadow-sm ring-1 ring-background"
          style={{ left: `${pct}%` }}
        />
      </div>
      <div className="mt-0.5 flex justify-between text-[7px] font-medium uppercase tracking-wide text-muted-foreground">
        <span>52W L</span>
        <span>52W H</span>
      </div>
    </div>
